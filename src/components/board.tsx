...
import type { Board, FilteredTrains, Station } from '@/types/stations'
...
export default function TrainTimesBoard({
  ...
  useEffect(() => {
    ...
    const announcementTimer = setInterval(() => {
      if (trains?.nrccMessages?.length === 0) {
        return
      }
  ...
  return (
    <div className='flex min-h-[calc(100vh-4rem)] w-full flex-col bg-background p-4 transition-colors duration-500'>
